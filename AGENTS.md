...
## 專案專屬
- Build/test 指令：無建置（單一 HTML）。測試：`npx playwright test`（真實 Chromium，44 條；測試在 `tests/` 下的 stitch／quality／touch／mobile／pwa／deep-debug 六個 spec；首次需 `npx playwright install chromium`）。本機直接用瀏覽器開 `index.html` 即可執行。
